...
export function apply(ctx: Context, config: Config) {
  ...
  const handleGroupRequest = async (session: any) => {
    ...
    let reason = ''
    let rejectionMessage = config.groupManagement.rejectionMessage

    const message = session.content || session.comment || ''

    // 优先级1: 严格姓名验证模式
    if (config.groupManagement.useNameValidation) {
      const hasMatchingName = config.nameWhitelist.some(name => 
  ...
  // QQ号白名单管理命令
  ctx.command('whitelist', 'QQ号白名单管理')
    .action(async () => {
      const helpMessage = [
        '📋 QQ号白名单管理命令:',
        '',
  ...
  ctx.command('whitelist.quick <qq:string>', '快速添加QQ号到白名单（智能解析）')
    .action(async (_, qq) => {
      if (!qq) {
        return '❌ 请提供QQ号'
      }
      ...
      // 智能解析QQ号，去除可能的前缀和后缀
      const cleanedQq = qq.replace(/[^0-9]/g, '') // 只保留数字
      
      if (!cleanedQq || !/^\d{5,11}$/.test(cleanedQq)) {
        return [
          '❌ 无效的QQ号格式',
          '',
          '💡 支持的格式:',
      ...
      if (config.whitelist.includes(cleanedQq)) {
        return `⚠️ QQ号 ${cleanedQq} 已在白名单中`
      }

      config.whitelist.push(cleanedQq)
  ...
  ctx.command('names', '姓名白名单管理')
    .action(async () => {
      const helpMessage = [
        ...
        '• names.batch 张三 李四 王五',
        '',
        `当前严格验证状态: ${config.groupManagement.useNameValidation ? '✅ 启用' : '❌ 禁用'}`,
        `姓名白名单数量: ${config.nameWhitelist.length} 个`,
        '',
  ...
  ctx.command('names.add <name:string>', '添加姓名到白名单')
    .action(async (_, name) => {
      ...
      if (config.nameWhitelist.includes(trimmedName)) {
        return '⚠️ 该姓名已在白名单中'
      }

      config.nameWhitelist.push(trimmedName)
      return `✅ 已将 "${trimmedName}" 添加到姓名白名单`
    })

  ctx.command('names.quick <name:string>', '快速添加姓名到白名单（智能处理）')
    .action(async (_, name) => {
      if (!name) {
        return '❌ 请提供姓名'
      }
      ...
      // 智能清理姓名，去除多余空格和特殊字符
      const cleanedName = name.replace(/[^\u4e00-\u9fa5a-zA-Z\s]/g, '').trim()
      ...
      if (config.nameWhitelist.includes(cleanedName)) {
        return `⚠️ 姓名 "${cleanedName}" 已在白名单中`
      }

      config.nameWhitelist.push(cleanedName)
      return `✅ 已将姓名 "${cleanedName}" 添加到白名单`
    })

  ctx.command('names.batch <names:text>', '批量添加姓名到白名单')
    ...
    .action(async (_, names) => {
      ...
      nameList.forEach(name => {
        if (config.nameWhitelist.includes(name)) {
          results.duplicate.push(name)
        } else {
          results.success.push(name)
          config.nameWhitelist.push(name)
        }
      })

      // 生成结果报告
      const report = []
      report.push('📊 批量添加姓名白名单结果:')
      report.push('')

      if (results.success.length > 0) {
        report.push(`✅ 成功添加 ${results.success.length} 个:`)
        report.push(results.success.map(name => `• ${name}`).join('\n'))
        report.push('')
      }

      if (results.duplicate.length > 0) {
        report.push(`⚠️ 已存在 ${results.duplicate.length} 个:`)
        report.push(results.duplicate.map(name => `• ${name}`).join('\n'))
        report.push('')
      }

      report.push(`📋 当前姓名白名单总数: ${config.nameWhitelist.length}`)

      return report.join('\n')
    })

  ctx.command('names.remove <name:string>', '从姓名白名单中移除')
    .action(async (_, name) => {
      if (!name) {
        return '❌ 请提供姓名'
      }
      ...
      const trimmedName = name.trim()
      const index = config.nameWhitelist.indexOf(trimmedName)
      if (index === -1) {
        return '⚠️ 该姓名不在白名单中'
      }

      config.nameWhitelist.splice(index, 1)
      return `✅ 已将 "${trimmedName}" 从姓名白名单移除`
    })

  ctx.command('names.list', '查看姓名白名单')
    .action(async () => {
      if (config.nameWhitelist.length === 0) {
        return '👤 姓名白名单为空'
      }

      const list = config.nameWhitelist.map((name, index) => `${index + 1}. ${name}`).join('\n')
      return `👤 姓名白名单列表 (${config.nameWhitelist.length} 个):\n\n${list}`
    })

  ctx.command('names.clear', '清空姓名白名单')
    .action(async () => {
      const count = config.nameWhitelist.length
      config.nameWhitelist.length = 0
      return `✅ 已清空姓名白名单 (移除了 ${count} 个姓名)`
    })

  ctx.command('names.validation-toggle', '切换严格姓名验证模式')
    .action(async () => {
      config.groupManagement.useNameValidation = !config.groupManagement.useNameValidation
      return [
        `🔄 严格姓名验证模式已${config.groupManagement.useNameValidation ? '启用' : '禁用'}`,
        '',
        ...
        '',
        config.groupManagement.useNameValidation && config.nameWhitelist.length === 0
